...
for id in `jq '.[].id' archivedAppData.json`; do 
        publicApplicationId_appjson=`jq .publicApplicationId webos-catalog-metadata/$id.json | sed 's/\"//g'`
        version_appjson=`jq .version webos-catalog-metadata/$id.json | sed 's/\"//g'`
        ...
        appSize_appjson=`jq .appSize webos-catalog-metadata/$id.json | sed 's/\"//g'`
        filename_appjson=`jq .filename webos-catalog-metadata/$id.json | sed 's/\"//g'`
        title=`jq '.[] | select(.id=='$id') | .title' archivedAppData.json | sed 's/\"//g' | tail -1`
        homeURL_appjson=`jq .homeURL webos-catalog-metadata/$id.json | sed 's/\"//g'`
	lastModifiedTime_appjson=`jq .lastModifiedTime webos-catalog-metadata/$id.json | sed 's/\"//g'`
	modified_date=`date -j -f "%Y-%m-%dT%H:%M:%S" $lastModifiedTime_appjson +%s 2>/dev/null`
        appIcon=`jq '.[] | select(.id=='$id') | .appIcon' archivedAppData.json | sed 's/\"//g' | tail -1`
        description_appjson=`jq .description webos-catalog-metadata/$id.json | sed 's/\"//g' |  sed 's/\\.//g' | head -1`
        screenshot_appjson=`jq .[] webos-catalog-metadata/$id.json | jq .[].screenshot 2>/dev/null | grep -v ^null | sed 's/^\"/\"http\:\/\/museum.weboslives.eu\/AppImages\//g' | tr '\n' ',' | sed 's/,$//g'`
        locale_appjson=`jq .locale webos-catalog-metadata/$id.json | sed 's/\"//g'`
        licenseURL_appjson=`jq .licenseURL webos-catalog-metadata/$id.json | sed 's/\"//g'`
