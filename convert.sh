...
# This is a horrible script, the whole thing will need to be rewritten
#
# Pre-requisites:
#
#  - Install jq sed awk grep (GNU versions preferably)
...
for id in `jq '.[].id' archivedAppData.json`; do 
        ...
        category=`jq '.[] | select(.id=='$id') | .category' archivedAppData.json | sed 's/\"//g'`
        author=`jq '.[] | select(.id=='$id') | .author' archivedAppData.json | sed 's/\"//g'`
        ...
        title=`jq '.[] | select(.id=='$id') | .title' archivedAppData.json | sed 's/\"//g'`
        homeURL_appjson=`jq .homeURL webos-catalog-metadata-main/$id.json | sed 's/\"//g'`
        appIcon=`jq '.[] | select(.id=='$id') | .appIcon' archivedAppData.json | sed 's/\"//g'`
        description=`jq '.[] | select(.id=='$id') | .description' archivedAppData.json | sed 's/\"//g'`
        screenshot_appjson=`jq .[] webos-catalog-metadata-main/$id.json | jq .[].screenshot 2>/dev/null | grep -v ^null | sed 's/^\"/\"http\:\/\/appimages.webosarchive.com\//g' | sed -z 's/\n/,/g;s/,$/\n/'`
        ...
        echo 'Source: {"Title":"'$title'","Category":"'$category'","Homepage":"'$homeURL_appjson'","Icon":"http://appimages.webosarchive.com/'$appIcon'","FullDescription":"'$description'","Screenshots":['$screenshot_appjson'],"Countries":["'$locale_appjson'"],"Languages":["'$locale_appjson'"],"License":"'$licenseURL_appjson'","DeviceCompatibility":['$devices']}'
